...
def staff_home(request):
    ...
    students_count = Students.objects.filter(course_id__in=final_course)

    for stud_id in students_count:
        if stud_id not in stud:
            stud.append(stud_id)

    stud_result=StudentResult.objects.filter(student_id__in=stud)
    ...
    if request.method != "POST":
        ...
        try:
            ...
            if check_exist:
                ...
                result.t1=int(t1)
                result.t2=int(t2)
                result.seminar=int(seminar)
                result.subject_exam_marks = int(int(t1)+int(t2))
                result.total=int(int(result.seminar)+int(result.subject_assignment_marks)+int(result.subject_exam_marks))
                result.save()
                messages.success(request, "Result Updated Successfully!")
                return redirect('staff_add_result')
            else:
                result = StudentResult(student_id=student_obj, subject_id=subject_obj, subject_exam_marks= int(int(t1)+int(t2)), subject_assignment_marks=assignment_marks,t1=t1,t2=t2,seminar=seminar,total=int(int(seminar)+int(assignment_marks)+int(int(t1)+int(t2))))
                result.save()
...
def report_card(request, id):
    students_count = StudentResult.objects.get(pk=id)
...
class ReportCardView(View):
    def get(self, request, id):
        ...
        ws = wb.add_sheet('Report Card')

        # Write column headers
        headers = ['Student ID', 'Student name', 'Subject Exam Marks', 'T1', 'T2', 'Attendance', 'Seminar', 'Subject Assignment Marks',  'Total']
        ...
        row = [
            ...
            result.student_id.admin.first_name,
            result.subject_exam_marks,
            result.t1,
            result.t2,
            result.attendance,
            result.seminar,
            result.subject_assignment_marks,
            result.total,
        ]
        for col_num, cell_value in enumerate(row):
            ws.write(1, col_num, cell_value)

        # Write feedback row
        if result.total<50:
            feedback = f'{result.student_id.admin.first_name}-{result.student_id.admin.last_name},need improvement'
        if result.total>50:
            feedback = f'{result.student_id.admin.first_name}-{result.student_id.admin.last_name},Performance is good'
        ws.write(3, 0, 'Feedback:')
        ws.write(3, 1, feedback)

        # Set response headers and content type
        response = HttpResponse(content_type='application/ms-excel')
        response['Content-Disposition'] = 'attachment; filename=report_card.xls'

        # Write workbook to response and return it
        wb.save(response)
        return response
